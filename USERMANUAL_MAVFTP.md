...
```powershell
python .\ardupilot_methodic_configurator\frontend_tkinter_download_bin_logs.py
```
...
- **Download last .bin log file** downloads the latest flight log using the
  flight controller's log mechanism.
...
- **Close** closes the browser. During an active transfer it becomes
  **Cancel operation**.
...
The browser shows a progress window for remote transfers. Click **Cancel
operation** to request cancellation. The current MAVFTP operation stops at its
next safe progress point; completed entries remain completed.
