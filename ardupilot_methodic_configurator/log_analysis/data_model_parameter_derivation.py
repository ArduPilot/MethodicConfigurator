...
class ParameterDeriver(Protocol):
    ...
    def derived_and_forced_parameters_matching(
        self, pattern: str, *, configuration_steps: dict[str, Any]
    ) -> dict[str, str]:
...
class ConfigurationStepParameterDeriver:
    ...
    def expected_parameter_value(
        ...
        *,
        ...
        if step_filename not in configuration_steps:
            return None, ""

        eval_variables: dict[str, Any] = {"vehicle_components": vehicle_components}
        if apm_doc:
            eval_variables["doc_dict"] = apm_doc
        if parameters:
            eval_variables["fc_parameters"] = parameters

        step_dict = configuration_steps[step_filename]
    ...
    def derived_and_forced_parameters_matching(
        ...
        result: dict[str, str] = {}
        for step_filename, step_info in configuration_steps.items():
