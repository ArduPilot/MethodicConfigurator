...
class ParameterEditorWindow(BaseWindow):  # pylint: disable=too-many-instance-attributes
    ...
    def __should_copy_fc_values_to_file_old(self, selected_file: str) -> None:
        ...
        if auto_changed_by and self.flight_controller.fc_parameters:
            msg = _(
                ...
                "The experiment procedure is described in the tunning guide.\n\n"
                "Choose an option:\n"
                ...
                " CANCEL - Exit the application and go perform the experiment"
            )
            response = messagebox.askyesnocancel(
                _("Update file with values from FC?"), msg.format(**locals()), default=messagebox.CANCEL
            )
            ...
            elif response is None:  # Quit option
                sys.exit(0)
    ...
    def __should_copy_fc_values_to_file(self, selected_file: str) -> None:
        ...
        if auto_changed_by and self.flight_controller.fc_parameters:
            ...
            # Result variable
            result = [None]  # Use a list to store the result since nonlocal isn't needed for lists
            ...
            dialog.bind("<Return>", lambda event: [result.append(None), dialog.destroy()])
