...
class ParameterEditorWindow(BaseWindow):  # pylint: disable=too-many-instance-attributes
    ...
    def _update_previous_button_state(self) -> None:
        ...
        if not hasattr(self, "previous_button"):
            return
        parameter_files = self.file_selection_combobox["values"]
        try:
            current_file_index = parameter_files.index(self.file_selection_combobox.get())
        except ValueError:
            current_file_index = 0
        self.previous_button.configure(state="normal" if current_file_index > 0 else "disabled")
    ...
    def on_previous_click(self, _event: Union[tk.Event, None] = None) -> None:  # noqa: UP007
        ...
        self.write_changes_to_intermediate_parameter_file()
        self.file_selection_combobox.set(parameter_files[current_file_index - 1])
