...
class DownloadBinLogsWindow(  # pylint: disable=attribute-defined-outside-init, too-many-instance-attributes
    ...
    def _build_widgets(self) -> None:
        ...
        last_log_button.grid(row=0, column=0, padx=3)
        ...
        download_button.grid(row=0, column=1, padx=3)
        show_tooltip(
        ...
        upload_button.grid(row=0, column=2, padx=3)
        show_tooltip(
            upload_button, _("Upload selected files and directories from the local computer to the flight controller")
        )

        cancel_button = ttk.Button(action_frame, text=_("Close"), command=self._on_cancel_or_close, width=action_button_width)
        cancel_button.grid(row=0, column=3, padx=3)
        ...
        try:
            ...
            for child in children:
                ...
                try:
                    ...
                    continue
                entries.append(LocalFileEntry(child.name, child, size_bytes, is_directory, stat.st_mtime))
    ...
    def _start_remote_task(
        ...
        root: tk.Misc | None = getattr(self, "root", None)
        ...
        root.after(50, self._poll_remote_task)
        return True
    ...
    def _download_remote_plan_worker(
        ...
        for entry, target in plan.files:
            ...
            def report_file_progress(
                ...
                progress = min(total, int(offset + size * (current / maximum if maximum else 0.0)))
    ...
    def create_new_remote_directory(self) -> None:
        ...
        def worker(
            ...
            if cancel_event.is_set():
                return [], [], True
            success = self._call_remote_bool(self.parameter_editor.make_remote_directory, new_path)
            return ([new_path], [], False) if success else ([], [new_path], False)

        def completion(succeeded: list[str], failed: list[str], cancelled: bool) -> None:
            if cancelled:
                self._show_summary(_("New directory summary"), succeeded, failed, cancelled)
            elif succeeded:
    ...
    def download_last_flight_log(self) -> None:
        ...
        def worker(
            ...
            if cancel_event.is_set():
                return [], [], True
            return ([filename], [], False) if success else ([], [filename], False)
...
def _standalone_ui_services() -> DownloadBinLogsUiServices:  # pragma: no cover
