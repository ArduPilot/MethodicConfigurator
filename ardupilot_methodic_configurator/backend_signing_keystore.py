...
@dataclass
class StoredKey:  # pylint: disable=too-many-instance-attributes
    ...
    description: str = ""
    storage_location: str = "file"  # "file" or "keyring"
    last_rotated_at: str = ""  # ISO 8601 timestamp of last key rotation
...
class SigningKeystore:
    ...
    ⚠️  **CRITICAL SECURITY LIMITATION - READ CAREFULLY** ⚠️

    The file-based fallback storage uses vehicle_id as the encryption key derivation input.
    **THIS PROVIDES ONLY OBFUSCATION, NOT REAL ENCRYPTION!**

    **Security Implications:**
    - Anyone with filesystem access can decrypt your signing keys
    - vehicle_id typically provides only 20-40 bits of entropy (easily brute-forced)
    - Same vehicle_id produces same encryption key across all installations
    - This is NOT suitable for high-security environments

    **STRONGLY RECOMMENDED:**
    1. Use OS keyring when available (Windows Credential Manager, macOS Keychain, Linux Secret Service)
    2. Restrict filesystem permissions (this code sets 0o600 on Unix)
    3. Consider this fallback only for development/testing, NOT production
    4. For production, implement additional master password protection
    ...
    **You have been warned!**
    ...
    def __init__(self, use_keyring: bool = True) -> None:
        ...
        try:
            import cryptography  # noqa: F401, PLC0415 # pylint: disable=import-outside-toplevel,unused-import
        except ImportError as exc:
            msg = "cryptography package is required for MAVLink signing. Install it with: pip install cryptography"
        ...
        if not self._keyring_available:
            self._logger.warning(
                "⚠️  OS keyring not available - falling back to WEAK file-based encryption. "
                "File encryption uses vehicle_id (low entropy) and provides only obfuscation. "
                "Anyone with filesystem access can decrypt signing keys! "
                "For production use, install and configure OS keyring. See INSTALL.md for details."
            )
    ...
    def _check_keyring_available(self) -> bool:
        ...
        try:
            ...
            self._logger.warning("Keyring package not installed")
            return False
    ...
    def store_key(self, vehicle_id: str, key: bytes, description: str = "") -> bool:
        ...
        # Check if key already exists and warn about overwrite
        # Check metadata first (fast, no decryption) instead of full retrieve
        try:
            keystore = self._load_keystore_file()
            if vehicle_id in keystore.keys:
                self._logger.warning(
                    "Overwriting existing signing key for vehicle %s. "
                    "Previous key will be lost! Consider using rotate_key() instead.",
                    vehicle_id,
                )
        except Exception as exc:  # pylint: disable=broad-except
            self._logger.debug("Could not check for existing key: %s", exc)
        ...
        if self._keyring_available:
            try:
                ...
                # Store metadata in file to track storage location
                self._store_key_metadata(vehicle_id, description, "keyring")
                return True
            except Exception as exc:  # pylint: disable=broad-except
        ...
        self._logger.warning(
            "Storing key for vehicle %s in file with WEAK encryption (vehicle_id-based). "
            "This provides only obfuscation, not real security!",
            vehicle_id,
        )
        ...
        if self._keyring_available:
            try:
                ...
                if key_b64:
                    ...
                    if len(key) != SIGNING_KEY_LENGTH:
                        self._logger.error(
                            "Retrieved key for vehicle %s has invalid length: %d (expected %d). Key may be corrupted.",
                            vehicle_id,
                            len(key),
                            SIGNING_KEY_LENGTH,
                        )
    ...
    def _retrieve_key_from_file(self, vehicle_id: str) -> Optional[bytes]:
        ...
        try:
            ...
            # Validate key length
            if len(decrypted_key) != SIGNING_KEY_LENGTH:
                self._logger.error(
                    "Decrypted key for vehicle %s has invalid length: %d (expected %d). "
                    "Key may be corrupted or file may be tampered with.",
                    vehicle_id,
                    len(decrypted_key),
                    SIGNING_KEY_LENGTH,
                )
    ...
    def delete_key(self, vehicle_id: str) -> bool:
        ...
        # Check metadata first to determine storage location (if available)
        storage_location = self._get_storage_location(vehicle_id)

        # If storage location is unknown and keyring is available, still try keyring
        # This handles cases where keys were stored before metadata tracking
        if (storage_location in ("keyring", "unknown")) and self._keyring_available:
            try:
                import keyring  # noqa: PLC0415 # pylint: disable=import-outside-toplevel
        ...
        # Delete from file (either the key itself or just metadata)
        try:
            keystore = self._load_keystore_file()
            if vehicle_id in keystore.keys:
                ...
                self._save_keystore_file(keystore)
                deleted = True
                if storage_location != "keyring":
                    self._logger.info("Deleted signing key for vehicle %s from file", vehicle_id)
                else:
                    self._logger.debug("Deleted metadata for vehicle %s from file", vehicle_id)
        except Exception as exc:  # pylint: disable=broad-except
            self._logger.debug("Failed to delete from file: %s", exc)
    ...
    def _save_keystore_file(self, keystore: KeystoreData) -> None:
        ...
        lock_path = self._fallback_path.with_suffix(".lock")
    ...
    def _get_storage_location(self, vehicle_id: str) -> str:
        """
        Get the storage location for a vehicle's key.

        Args:
            vehicle_id: Vehicle identifier

        Returns:
            str: "keyring" or "file" or "unknown" if not found

        """
        try:
            keystore = self._load_keystore_file()
            if vehicle_id in keystore.keys:
                return keystore.keys[vehicle_id].storage_location
        except Exception as exc:  # pylint: disable=broad-except
            self._logger.debug("Failed to get storage location: %s", exc)
        return "unknown"

    def _store_key_metadata(self, vehicle_id: str, description: str, storage_location: str) -> None:
        """
        Store key metadata when key is stored in keyring.

        Args:
            vehicle_id: Vehicle identifier
            description: Key description
            storage_location: Where the key is stored ("keyring" or "file")
        ...
        try:
            ...
            stored_key = StoredKey(
                ...
                vehicle_id=vehicle_id,
                created_at=datetime.now(tz=timezone.utc).isoformat(),
                encrypted_key="",  # Empty for keyring storage
                salt="",  # Empty for keyring storage
                description=description,
            ...
            self._logger.warning("Failed to store key metadata: %s", exc)
    ...
    def rotate_key(self, vehicle_id: str) -> Optional[bytes]:
        ...
        Returns:
            bytes: The new signing key if successful, None otherwise
        ...
        try:
            ...
            description = ""
            if vehicle_id in keystore.keys:
                description = keystore.keys[vehicle_id].description
        except Exception as exc:  # pylint: disable=broad-except
            self._logger.debug("Failed to load existing metadata: %s", exc)
            description = "Rotated key"

        # Store new key
        if self.store_key(vehicle_id, new_key, description):
            # Update rotation timestamp
            try:
                keystore = self._load_keystore_file()
                if vehicle_id in keystore.keys:
                    keystore.keys[vehicle_id].last_rotated_at = datetime.now(tz=timezone.utc).isoformat()
                    self._save_keystore_file(keystore)
            except Exception as exc:  # pylint: disable=broad-except
                self._logger.warning("Failed to update rotation timestamp: %s", exc)

            self._logger.info("Successfully rotated signing key for vehicle %s", vehicle_id)
            return new_key

        self._logger.error("Failed to rotate signing key for vehicle %s", vehicle_id)
        return None
