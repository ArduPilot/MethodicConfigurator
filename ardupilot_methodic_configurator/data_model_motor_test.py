...
class MotorTestDataModel:  # pylint: disable=too-many-public-methods
    ...
    def get_frame_options(self) -> dict[str, dict[int, str]]:
        """
        Get available frame classes and types from parameter metadata.

        Returns:
            dict[str, dict[int, str]]: A dictionary of frame options.

        """
        frame_options: dict[str, dict[int, str]] = {}

        # Get FRAME_CLASS options
        frame_class_options = {}
        if hasattr(self.filesystem, "doc_dict") and "FRAME_CLASS" in self.filesystem.doc_dict:
            frame_class_values = self.filesystem.doc_dict["FRAME_CLASS"].get("values", {})
            for code, name in frame_class_values.items():
                if code is not None:
                    try:
                        frame_class_options[int(code)] = name
                    except (ValueError, TypeError):
                        continue

        # Get FRAME_TYPE options for each frame class
        if hasattr(self.filesystem, "doc_dict") and "FRAME_TYPE" in self.filesystem.doc_dict:  # pylint: disable=too-many-nested-blocks
    ...
    def get_motor_directions(self) -> list[str]:
        ...
        """
        # Find the layout matching current frame class and type in the loaded motor data
        if not self._motor_data or "layouts" not in self._motor_data:
            logging_error(_("Motor data not available"))
            return [""]

        # Search for matching layout
        for layout in self._motor_data["layouts"]:
            if layout["Class"] == self._frame_class and layout["Type"] == self._frame_type:
                # Extract motor directions in motor number order
                motors = layout["motors"]
                # Sort by motor number to ensure correct order
                sorted_motors = sorted(motors, key=lambda m: m["Number"])

                directions = [motor["Rotation"] for motor in sorted_motors]

                # Validate that we have the expected number of motors
                if len(directions) != self._motor_count:
                    logging_warning(
                        _("Motor count mismatch: expected %(expected)d, found %(found)d for frame %(class)d/%(type)d"),
                        {
                            "expected": self._motor_count,
                            "found": len(directions),
                            "class": self._frame_class,
                            "type": self._frame_type,
                        },
                    )
                    # Extend or truncate to match expected motor count
                    if len(directions) < self._motor_count:
                        # Extend with alternating pattern
                        additional_directions = [
                            "CW" if i % 2 == 0 else "CCW" for i in range(len(directions), self._motor_count)
                        ]
                        directions.extend(additional_directions)
                    elif len(directions) > self._motor_count:
                        directions = directions[: self._motor_count]

                return directions

        # If no matching layout found, log error use default pattern
        logging_error(
            _("No motor layout found for frame class %(class)d, type %(type)d."),
