...
class BaseWindow:
    ...
    def _setup_theme_and_styling(self) -> None:
        ...
        bold_font_size = self.calculate_scaled_font_size(10)
    ...
    def put_image_in_label(
        ...
        try:
            ...
            if not filepath:
                msg = "Image filepath cannot be empty"
            ...
            if not os.path.isfile(filepath):
                msg = f"Image file not found: {filepath}"
                raise FileNotFoundError(msg)

            # Load and validate the image
            image = Image.open(filepath)
            if image is None:
                msg = f"Failed to load image from {filepath}"
                raise ValueError(msg)

            # Calculate new dimensions while preserving aspect ratio
            width, height = image.size
            if height == 0:
                msg = "Image has zero height"
                raise ValueError(msg)
            ...
            # Resize the image
            resized_image = image.resize((new_width, image_height), Image.Resampling.LANCZOS)
