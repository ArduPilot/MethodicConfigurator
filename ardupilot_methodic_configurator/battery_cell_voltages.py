...
BATTERY_DEFAULT_CHEMISTRY = "Lipo"
...
class BatteryCell:
    ...
    def limit_max_voltage(chemistry: str) -> float:
        max_abs_max = max(chem["absolute_max"] for chem in _recommended_battery_cell_voltages.values())
    ...
    def limit_min_voltage(chemistry: str) -> float:
        min_abs_min = min(chem["absolute_min"] for chem in _recommended_battery_cell_voltages.values())
        if chemistry not in _recommended_battery_cell_voltages:
            return min_abs_min
