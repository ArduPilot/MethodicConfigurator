...
def configuration_steps_strings() -> None:  # noqa: PLR0915 # pylint: disable=too-many-statements
    ...
    """
    # why
    _config_steps_strings = _("ATC_THR_MIX_MAX should be increased after autotune to maximize attitude control authority at high throttle")
    _config_steps_strings = _("After calibrating the IMU temperature bias, disable disarmed logging (no longer needed) and set board target temperature.")
    _config_steps_strings = _("An OSD (On Screen Display) overlays flight data on the FPV video feed, giving the pilot situational awareness without looking away from the aircraft")
    ...
    _config_steps_strings = _("Configuring RC controller options ensures correct arming behavior and channel function assignments for the specific RC system in use")
    ...
    _config_steps_strings = _("Disarmed and replay logging must be disabled after windspeed estimation flights to avoid filling the storage with unnecessary logs")
    ...
    _config_steps_strings = _("The D feed-forward term improves aggressive rate tracking and gives the drone a 'locked-in' feel with minimal control latency")
    _config_steps_strings = _("The IMU drift is temperature dependent and can cause gyro and/or accel inconsistent errors")
    _config_steps_strings = _("The PID notch filter(s) configuration depends on real-flight data collected with the logging enabled in the previous step")
    _config_steps_strings = _("The VTOL-quicktune lua script can safely estimate good PID starting values for the vehicle.")
    _config_steps_strings = _("The battery monitor must be configured to match the hardware connection type so that the autopilot correctly reads battery voltage and current")
    ...
    # why_now
    _config_steps_strings = _("After all autotune axes are complete and PID values are finalized, since D_FF is calculated from the final tuned PID response")
    _config_steps_strings = _("After all autotune flights are complete and the PIDs are finalized")
    _config_steps_strings = _("After the RC receiver protocol is configured, the controller-specific options such as arming method and channel function can be set")
    _config_steps_strings = _("After the battery voltage and failsafe thresholds are set, the monitor hardware connection parameters can be configured")
    ...
    _config_steps_strings = _("After the first flight because it depends on the Q_M_THST_HOVER parameter, before the second flight so that it can safely use the altitude controller")
    _config_steps_strings = _("All the previous steps did not require logging, but the following ones do")
    _config_steps_strings = _("Because LOG_DISARMED and LOG_REPLAY were enabled for windspeed estimation and barometer compensation flights and are no longer needed")
    ...
    _config_steps_strings = _("Before the first flight so that arming checks are enforced and the vehicle responds safely to abnormal conditions")
    ...
    _config_steps_strings = _("Can only be done after IMU temperature compensation results are available. Future steps will use a drift-compensated, constant temperature and only log when armed.")
    ...
    _config_steps_strings = _("Real-flight data with PID notch filter batch logging is only available after the dedicated logging flight")
    ...
    _config_steps_strings = _("Configure parameters for the pitch axis input system identification to create a mathematical model of the vehicle")
    ...
    _config_steps_strings = _("Configure parameters for the roll axis input system identification to create a mathematical model of the vehicle")
    ...
    _config_steps_strings = _("Configure parameters for the yaw axis input system identification to create a mathematical model of the vehicle")
    ...
    _config_steps_strings = _("Configure safety parameters including arming checks, geofence, failsafe actions and ESC slew rate limits")
    ...
    _config_steps_strings = _("Configure the On Screen Display (OSD) to show relevant flight data on the FPV video feed")
    _config_steps_strings = _("Configure the PID notch filter(s) based on the data collected from the logging flight")
    _config_steps_strings = _("Configure the battery monitor hardware connection type, protocol and calibration values")
    ...
    _config_steps_strings = _("Finalize autotune by setting the maximum throttle mix parameter")
    _config_steps_strings = _("Finish IMU (Inertial Measurement Unit) temperature calibration")
    ...
    _config_steps_strings = _("Restore logging parameters to normal after windspeed estimation flights")
    ...
    _config_steps_strings = _("Use Q_M_THST_HOVER value calculated during the first flight to set throttle controller PIDs")
    _config_steps_strings = _("Use optical flow sensor instead of GNSS sensor for positioning")

    # wiki_text
    _config_steps_strings = _("Arming Checks")
    ...
    _config_steps_strings = _("OSD (On Screen Display)")
    ...
    _config_steps_strings = _("Suppress frame resonance with PID notch filters")
...
Jump to '60_position_controller.param' file?""")
    ...
    _config_steps_strings = _("After a successful IMU temperature calibration, the FC will automatically set this to 1")
    ...
    _config_steps_strings = _("Because ALTHOLD flight mode was used for more than 30 seconds to correctly learn the Q_M_THST_HOVER value")
    _config_steps_strings = _("Critical failsafe voltage x nr. of cells")
    _config_steps_strings = _("Defined in ESC specifications above")
    ...
    _config_steps_strings = _("For safety reasons, only start takeoff if all motors spin above this RPM")
    ...
    _config_steps_strings = _("IMU temperature calibration is finished, we no longer need to log data when disarmed")
    _config_steps_strings = _("INS_GYRO_FILTER / 2")
    _config_steps_strings = _("Initial value, will be improved at a later step")
    _config_steps_strings = _("Inject chip on the input pitch signal")
    _config_steps_strings = _("Inject chip on the input roll signal")
    _config_steps_strings = _("Inject chip on the input yaw signal")
    ...
    _config_steps_strings = _("Log relevant data for PID notch filters tuning. Later on we'll change this to other subsystems")
    _config_steps_strings = _("Log relevant data for dynamic notch filter tuning. Later on we'll change this to other subsystems")
    _config_steps_strings = _("Low failsafe voltage x nr. of cells")
    _config_steps_strings = _("MOT_SPIN_ARM plus at least 0.03")
    _config_steps_strings = _("Maximize attitude control authority at high throttle")
    _config_steps_strings = _("No more system identification chip injections")
    _config_steps_strings = _("No need to log raw data for IMU temperature calibration")
    _config_steps_strings = _("Normal state for everyday use")
    _config_steps_strings = _("Now that IMU temperature is calibrated, arm ASAP")
    ...
    _config_steps_strings = _("Q_M_SPIN_ARM plus at least 0.03")
    ...
    _config_steps_strings = _("RC receiver on SERIAL1 requires RCIN")
    _config_steps_strings = _("RC receiver on SERIAL2 requires RCIN")
    _config_steps_strings = _("RC receiver on SERIAL3 requires RCIN")
    _config_steps_strings = _("RC receiver on SERIAL4 requires RCIN")
    _config_steps_strings = _("RC receiver on SERIAL5 requires RCIN")
    _config_steps_strings = _("RC receiver on SERIAL6 requires RCIN")
    _config_steps_strings = _("RC receiver on SERIAL7 requires RCIN")
    _config_steps_strings = _("RC receiver on SERIAL8 requires RCIN")
    _config_steps_strings = _("RC receiver on SERIAL9 requires RCIN")
    ...
    _config_steps_strings = _("Telemetry on SERIAL1 protocol defined in component editor")
    _config_steps_strings = _("Telemetry on SERIAL2 protocol defined in component editor")
    _config_steps_strings = _("Telemetry on SERIAL3 protocol defined in component editor")
    _config_steps_strings = _("Telemetry on SERIAL4 protocol defined in component editor")
    _config_steps_strings = _("Telemetry on SERIAL5 protocol defined in component editor")
    _config_steps_strings = _("Telemetry on SERIAL6 protocol defined in component editor")
    _config_steps_strings = _("Telemetry on SERIAL7 protocol defined in component editor")
    _config_steps_strings = _("Telemetry on SERIAL8 protocol defined in component editor")
    _config_steps_strings = _("Telemetry on SERIAL9 protocol defined in component editor")
    _config_steps_strings = _("The default is 20Hz but that is too high in most situations")
    _config_steps_strings = _("The maximum value at which the motors spin")
    _config_steps_strings = _("The minimum value at which the motors start spinning reliably")
    _config_steps_strings = _("The vehicle TOW weight does not change, so no need to save it")
    _config_steps_strings = _("Throttle exponential curve from datasheet or from motor thrust stand")
    _config_steps_strings = _("To correctly scale the PID gains. Voltage of a healthy, fully charged battery immediately after takeoff. Read from a flight log")
    _config_steps_strings = _("Total battery capacity specified in the component editor")
    _config_steps_strings = _("Tuned by quiktune")
    _config_steps_strings = _("Use 0.1 * MOT_THST_HOVER assuming MOT_THST_HOVER has been correctly learned")
    _config_steps_strings = _("Use 0.1 * Q_M_THST_HOVER assuming Q_M_THST_HOVER has been correctly learned")
    _config_steps_strings = _("Use 0.2 * MOT_THST_HOVER assuming MOT_THST_HOVER has been correctly learned")
    _config_steps_strings = _("Use 0.2 * Q_M_THST_HOVER assuming Q_M_THST_HOVER has been correctly learned")
    ...
    _config_steps_strings = _("Use 2 * Q_M_THST_HOVER assuming Q_M_THST_HOVER has been correctly learned")
    ...
    _config_steps_strings = _("Use Q_M_THST_HOVER assuming Q_M_THST_HOVER has been correctly learned")
    ...
    _config_steps_strings = _("Use lua scripting for MagFit flight path automation")
    ...
    _config_steps_strings = _("Value for the first couple of flights will be changed later once Q_M_THST_HOVER is learned")
