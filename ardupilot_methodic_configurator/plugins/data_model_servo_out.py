...
_TRICOPTER_FRAME_CLASS = 7
_TRICOPTER_YAW_MOTOR_NUMBER = 7
_TRICOPTER_YAW_OUTPUT_NUMBER = 7
...
class ServoOutDataModel:
    ...
    def get_recommendations(self) -> tuple[dict[str, int], str, ServoOutRecommendationStatus]:
        ...
        tricopter_yaw_function = self._motor_function(_TRICOPTER_YAW_MOTOR_NUMBER)
        pinned_outputs = (
            {tricopter_yaw_function: _TRICOPTER_YAW_OUTPUT_NUMBER}
            if frame_class == _TRICOPTER_FRAME_CLASS and _TRICOPTER_YAW_OUTPUT_NUMBER in output_numbers
            else {}
        )
        recommendations = self._recommend_assignments(output_numbers, motor_numbers, existing, pinned_outputs)
        ...
        if recommendations:
            message = self._recommended_message(
                len(recommendations),
                tricopter_yaw_servo=(
                    frame_class == _TRICOPTER_FRAME_CLASS and tricopter_yaw_function in recommendations.values()
                ),
            )
    ...
    def _recommend_assignments(
        ...
        motor_numbers: tuple[int, ...],
        existing: Mapping[str, object],
        pinned: Mapping[int, int] | None = None,
    ) -> dict[str, int]:
        """Pair missing motor functions with free outputs, honoring pinned functions."""
        pinned = pinned or {}
        assigned_functions = {self._function_number(value) for value in existing.values()}
        free_output_numbers = [
            output_number
            for output_number in output_numbers
            if output_number not in pinned.values() and self._is_unset(existing.get(f"SERVO{output_number}_FUNCTION"))
        ]
        ...
        for motor_number in motor_numbers:
            ...
            if motor_function in assigned_functions:
                continue
            pinned_output = pinned.get(motor_function)
            if pinned_output is not None:
                if self._is_unset(existing.get(f"SERVO{pinned_output}_FUNCTION")):
                    recommendations[f"SERVO{pinned_output}_FUNCTION"] = motor_function
    ...
    def apply_recommendations(self) -> tuple[list[str], str, ServoOutRecommendationStatus]:
        ...
        recommendation_prefix = self._recommended_message(
            len(recommendations),
            tricopter_yaw_servo=self._get_frame_class() == _TRICOPTER_FRAME_CLASS
            and self._motor_function(_TRICOPTER_YAW_MOTOR_NUMBER) in recommendations.values(),
        )
