...
def _validate_windows_installer_url(url: str) -> bool:
    ...
    if not url.startswith("https://github.com/") and not url.startswith("https://objects.githubusercontent.com/"):
        logging_error(_("Windows installer URL must be from github.com or githubusercontent.com: %s"), url)
        return False
    return True
...
def download_and_install_pip_release(progress_callback: Optional[Callable[[float, str], None]] = None) -> int:
    """Download and install the latest release via pip from PyPI."""
    logging_info(_("Updating via pip for Linux and macOS..."))

    if progress_callback:
        progress_callback(0.0, _("Starting installation..."))

    # Install from PyPI with explicit index URL for validation
