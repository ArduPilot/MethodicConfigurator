...
- On flight controllers with an IOMCU, avoid using the IOMCU `MAIN` outputs
  for passthrough. Use supported `AUX` outputs controlled directly by the
  flight controller's main processor.
...
> Restore the saved ArduPilot parameter file after flashing ArduPilot(*_with_bl.hex) back onto the flight controller. Use the "Compare and Upload" button
