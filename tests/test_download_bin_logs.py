...
class TestDownloadBinLogsWindow:
    ...
    def test_remote_mtime_missing_from_older_firmware_is_shown_as_unsupported(self) -> None:
        ...
        assert DownloadBinLogsWindow._format_modified_time(None, unsupported_when_missing=True) == "Unsupported"
    ...
    def test_initial_remote_listing_falls_back_from_missing_log_directory(self) -> None:
        ...
        window._remote_task_thread = None

        def run_task(task, completion, **_kwargs) -> None:
            try:
                completion(task(None), None)
            except Exception as error:  # pylint: disable=broad-exception-caught
                completion(None, error)

        window._start_remote_task = run_task
    ...
    def test_operation_controls_use_ttk_state_flags(self) -> None:
        ...
        window._set_operation_controls_state("disabled")  # pylint: disable=protected-access
    ...
    def test_remote_listing_uses_worker_thread_and_tk_polling(self) -> None:
        ...
        window._operation_active = False
        window._remote_task_thread = None
        window._remote_task_queue = Queue()
        completion = MagicMock()

        with patch("ardupilot_methodic_configurator.frontend_tkinter_download_bin_logs.tk.Misc", FakeMisc):
            assert window._start_remote_task(lambda _event: "result", completion)  # pylint: disable=protected-access
            assert window._remote_task_thread is not None
            window._remote_task_thread.join(timeout=2)
            window._poll_remote_task()  # pylint: disable=protected-access

        completion.assert_called_once_with("result", None)

    def test_remote_listing_close_requests_cancellation(self) -> None:
        """Closing during a listing signals cancellation and explains the delay."""
        window = DownloadBinLogsWindow.__new__(DownloadBinLogsWindow)
        window._remote_task_thread = object()
        window._remote_task_cancel_event = Event()
        window.cancel_button = MagicMock()

        window._on_cancel_or_close()  # pylint: disable=protected-access

        assert window._remote_task_cancel_event.is_set()
        window.cancel_button.configure.assert_called_once()
    ...
    def test_background_operation_uses_worker_thread_and_tk_polling(self) -> None:
        ...
        window = DownloadBinLogsWindow.__new__(DownloadBinLogsWindow)
        window.root = FakeMisc()
        window._operation_queue = Queue()
        window._operation_active = False
        window._operation_thread = None
        window._operation_cancel_event = None
        window._operation_progress = MagicMock()
        window._operation_completion = None
        window.cancel_button = MagicMock()
        window.ui = MagicMock()
        window.ui.create_progress_window.return_value = MagicMock()
        completion = MagicMock()

        def worker(report_progress, _cancel_event) -> tuple[list[str], list[str], bool]:
            report_progress(1, 1)
            return ["done"], [], False

        with patch("ardupilot_methodic_configurator.frontend_tkinter_download_bin_logs.tk.Misc", FakeMisc):
            window._start_background_operation("title", "message", worker, completion)  # pylint: disable=protected-access
            assert window._operation_thread is not None
            window._operation_thread.join(timeout=2)
    ...
    def test_context_menu_selects_clicked_local_row_and_offers_new_directory(self) -> None:
        ...
        with patch("ardupilot_methodic_configurator.frontend_tkinter_download_bin_logs.tk.Menu", return_value=menu):
            result = window._show_context_menu(event, remote=False)  # pylint: disable=protected-access
        ...
        menu.add_command.assert_called_once_with(label="New directory", command=window.create_new_local_directory)
    ...
    def test_new_directory_is_selected_after_the_panel_refresh(self) -> None:
        ...
        window._select_entry_after_refresh("remote", "new-folder")  # pylint: disable=protected-access
        window._select_pending_entry("remote")  # pylint: disable=protected-access
