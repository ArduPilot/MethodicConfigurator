...
class TestDownloadBinLogsWindow:
    ...
    def test_initial_remote_listing_falls_back_from_missing_log_directory(self) -> None:
        ...
        def run_task(task, completion) -> None:
            try:
    ...
    def test_user_can_sort_remote_files_by_modification_time(self) -> None:
        """Clicking Modified sorts remote rows by their underlying timestamps."""
        window = DownloadBinLogsWindow.__new__(DownloadBinLogsWindow)
        window.remote_files = [
            FlightControllerLogFile("newest.BIN", "/APM/LOGS/newest.BIN", 1, modified_at=300),
            FlightControllerLogFile("oldest.BIN", "/APM/LOGS/oldest.BIN", 1, modified_at=100),
            FlightControllerLogFile("middle.BIN", "/APM/LOGS/middle.BIN", 1, modified_at=200),
        ]
        window.tree = MagicMock()
        window.tree.get_children.return_value = ("0", "1", "2")

        window._sort_by_column("modified", reverse=False)  # pylint: disable=protected-access
