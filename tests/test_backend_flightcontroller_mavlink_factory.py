...
"""
BDD-style tests for dependency injection services MavlinkConnectionFactory.
...
class TestMavlinkConnectionFactoryService:
    ...
    def test_system_mavlink_factory_handles_oserror(self) -> None:
        """
        SystemMavlinkConnectionFactory returns None when OSError occurs.

        GIVEN: mavutil.mavlink_connection raises OSError (device not found)
        WHEN: Creating connection
        THEN: Should return None instead of raising
        """
        # Given: Factory with mocked mavutil
        factory = SystemMavlinkConnectionFactory()

        with mock.patch(
            "ardupilot_methodic_configurator.backend_flightcontroller_mavlink_factory.mavutil.mavlink_connection"
        ) as mock_mavlink:
    ...
    def test_system_mavlink_factory_handles_timeouterror(self) -> None:
        """
        SystemMavlinkConnectionFactory returns None when TimeoutError occurs.
    ...
    def test_system_mavlink_factory_handles_valueerror(self) -> None:
        """
        SystemMavlinkConnectionFactory returns None when ValueError occurs.
