...
def create_forum_help_zip(parameter_editor: ParameterEditor) -> Path:
    """Call the refactored private API to create the forum help zip file."""
    return parameter_editor._create_forum_help_zip()


def get_forum_help_notification(parameter_editor: ParameterEditor, zip_path: Path) -> str:
    """Fetch the forum help notification text from the private API."""
    return parameter_editor._get_forum_help_notification(zip_path)


def open_forum_after_notification_dismissed(parameter_editor: ParameterEditor) -> None:
    """Invoke the refactored private browser helper."""
    parameter_editor._open_forum_after_notification_dismissed()
...
class TestZipVehicleForForumHelpUserNotification:
    ...
    def test_user_receives_notification_with_zip_path_and_filename(
        ...
        # Act: Create zip file and get notification details
        zip_path = create_forum_help_zip(parameter_editor)
        notification = get_forum_help_notification(parameter_editor, zip_path)
    ...
    def test_user_receives_notification_with_forum_upload_instructions(
        ...
        assert "https://discuss.ardupilot.org" in message
        assert "upload" in message.lower()
        # Verify zip was actually created
        assert zip_path.exists()
        # Verify .bin file instructions are included
    ...
    def test_user_receives_standard_info_notification(
        ...
        with patch(
            ...
            mock_webbrowser.assert_called_once_with("https://discuss.ardupilot.org")
        ...
        assert "https://discuss.ardupilot.org" in body
...
class TestZipVehicleForForumHelpBrowserAction:
    ...
    def test_user_gets_browser_opened_to_forum_after_acknowledging_notification(
        ...
        with patch(
            ...
            # Act: Create zip and simulate user dismissing notification
            _ = create_forum_help_zip(parameter_editor)  # zip_path not needed for this test
            open_forum_after_notification_dismissed(parameter_editor)

            # Assert: Verify browser was opened to correct URL
            mock_webbrowser.assert_called_once_with("https://discuss.ardupilot.org")
    ...
    def test_browser_opens_with_exact_forum_url(
        ...
        with patch(
            ...
            # Act: Create zip and trigger browser opening
            _ = create_forum_help_zip(parameter_editor)  # zip_path not needed for this test
            open_forum_after_notification_dismissed(parameter_editor)
...
class TestZipVehicleForForumHelpErrorHandling:
    ...
    def test_user_gets_error_message_when_zip_creation_fails_due_to_permissions(
        ...
        try:
            with pytest.raises((PermissionError, OSError)) as exc_info:
                create_forum_help_zip(parameter_editor)
    ...
    def test_user_gets_error_message_when_zip_creation_fails_due_to_disk_space(
        ...
        with pytest.raises(OSError, match=r".*space.*") as exc_info:
            create_forum_help_zip(parameter_editor)

        assert "space" in str(exc_info.value).lower()

    def test_browser_does_not_open_when_zip_creation_fails(self, parameter_editor: ParameterEditor, tmp_path: Path) -> None:
        """
        ...
        with patch(
            ...
            # Assert: Browser should not have been opened
            mock_webbrowser.assert_not_called()

    def test_user_gets_descriptive_error_for_missing_vehicle_directory(self, parameter_editor: ParameterEditor) -> None:
        """
        ...
        filesystem = get_filesystem(parameter_editor)
        filesystem.vehicle_dir = str(Path("/non/existent/path"))

        # Act & Assert: Verify appropriate error
...
class TestZipVehicleForForumHelpEmptyDirectory:
    ...
    def test_user_gets_error_when_no_parameter_files_exist(self, parameter_editor: ParameterEditor, tmp_path: Path) -> None:
        """
        ...
        # Act & Assert: Should raise error or create empty/minimal zip
        with pytest.raises((FileNotFoundError, ValueError)) as exc_info:
            create_forum_help_zip(parameter_editor)
...
class TestZipVehicleForForumHelpIntegration:  # pylint: disable=too-few-public-methods
    ...
    def test_user_can_complete_full_forum_help_workflow(
        ...
        # 3. User dismisses notification (simulated)
        with patch(
            "ardupilot_methodic_configurator.data_model_parameter_editor.webbrowser_open_url",
            autospec=True,
        ) as mock_webbrowser:
            ...
            # Zip exists with correct format
            assert zip_path.exists()
            ...
            # Notification has correct properties
            assert notification
            assert "https://discuss.ardupilot.org" in notification
...
class TestCreateForumHelpZipWorkflow:
    ...
    def test_workflow_notification_contains_zip_path_and_instructions(
        ...
        assert "https://discuss.ardupilot.org" in notification_message  # Forum URL
        assert ".bin" in notification_message  # Flight log mention

        # Browser already asserted explicitly in the patch context
