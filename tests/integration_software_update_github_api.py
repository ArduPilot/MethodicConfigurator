...
class TestRealGitHubAPIIntegration:
    ...
    def test_system_can_parse_sha256_checksums_from_real_release(self) -> None:
        ...
        THEN: Checksums are successfully parsed when available
        ...
        # Find a .exe or .whl file to test checksum retrieval
        test_files = [a for a in assets if a["name"].endswith((".exe", ".whl"))]

        if test_files:
            test_asset = test_files[0]
            checksum = get_expected_sha256_from_release(release_info, test_asset["name"])

            # If checksum is found, validate it
            if checksum:
