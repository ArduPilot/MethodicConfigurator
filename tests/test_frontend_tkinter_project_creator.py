...
class TestVehicleProjectCreatorWindow:
    ...
    def test_user_sees_open_error_after_template_project_creation(self, configured_creator_window, mock_messagebox) -> None:
        """
        An error opening a template-created project is shown by the creator window.

        GIVEN: The project manager reports an error while opening the new project
        WHEN: The user submits the template project creation form
        THEN: The error title and message are shown
        AND: The creator window remains open
        """
        window = configured_creator_window
        window.template_dir = MagicMock()
        window.template_dir.get_selected_directory.return_value = "/path/to/template"
