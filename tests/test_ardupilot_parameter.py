...
@pytest.fixture
def param_fixture() -> dict[str, Any]:
    ...
    # Forced parameter
    forced_param = ArduPilotParameter(param_name, par_obj, metadata, default_par, fc_value, is_forced=True)

    # Derived parameter
    derived_param = ArduPilotParameter(param_name, par_obj, metadata, default_par, fc_value, is_derived=True)
...
def test_basic_properties(param_fixture) -> None:
    ...
    assert param_fixture["full_param"].value == param_fixture["param_value"]
    assert param_fixture["full_param"].comment == param_fixture["param_comment"]
    assert param_fixture["full_param"].default_value == param_fixture["default_value"]
    assert param_fixture["full_param"].fc_value == param_fixture["fc_value"]
...
def test_value_comparisons(param_fixture) -> None:
    ...
    assert not param_fixture["full_param"].has_default_value
    ...
    assert default_value_param.has_default_value
...
def test_metadata_properties(param_fixture) -> None:
    ...
    assert param_fixture["full_param"].doc_tooltip == param_fixture["metadata"]["doc_tooltip"]
    assert param_fixture["full_param"].unit == param_fixture["metadata"]["unit"]
    assert param_fixture["full_param"].unit_tooltip == param_fixture["metadata"]["unit_tooltip"]
    ...
    assert param_fixture["basic_param"].doc_tooltip == "No documentation available in apm.pdef.xml for this parameter"
    assert param_fixture["basic_param"].unit == ""
    assert param_fixture["basic_param"].unit_tooltip == "No documentation available in apm.pdef.xml for this parameter"
...
def test_set_value(param_fixture) -> None:
    """Test setting parameter values."""
    # Regular parameter
    new_value = 12.0
    result = param_fixture["full_param"].set_value(new_value)
    assert result
    assert param_fixture["full_param"].value == new_value
    ...
    original_value = param_fixture["forced_param"].value
    result = param_fixture["forced_param"].set_value(new_value)
    assert not result
    assert param_fixture["forced_param"].value == original_value

    # Derived parameter - should return False and not change
    original_value = param_fixture["derived_param"].value
    result = param_fixture["derived_param"].set_value(new_value)
    assert not result
    assert param_fixture["derived_param"].value == original_value


def test_set_comment(param_fixture) -> None:
    """Test setting parameter comments."""
    # Regular parameter
    new_comment = "New comment"
    result = param_fixture["full_param"].set_comment(new_comment)
    assert result
    assert param_fixture["full_param"].comment == new_comment
    ...
    result = none_comment_param.set_comment("")
    assert not result

    # Forced parameter - should return False and not change
    original_comment = param_fixture["forced_param"].comment
    result = param_fixture["forced_param"].set_comment(new_comment)
    assert not result
    assert param_fixture["forced_param"].comment == original_comment

    # Derived parameter - should return False and not change
    original_comment = param_fixture["derived_param"].comment
    result = param_fixture["derived_param"].set_comment(new_comment)
    assert not result
    assert param_fixture["derived_param"].comment == original_comment
...
def test_is_valid_value(param_fixture) -> None:
    """Test value validation."""
    # Value within range
    assert param_fixture["full_param"].is_valid_value(10.0)

    # Value outside range
    assert not param_fixture["full_param"].is_valid_value(-1.0)
    assert not param_fixture["full_param"].is_valid_value(21.0)

    # Value at boundaries
    assert param_fixture["full_param"].is_valid_value(0.0)
    assert param_fixture["full_param"].is_valid_value(20.0)

    # Parameter without min/max constraints
    assert param_fixture["basic_param"].is_valid_value(-100.0)
    assert param_fixture["basic_param"].is_valid_value(100.0)
