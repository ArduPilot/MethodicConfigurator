...
def test_get_description(bitmask_dict) -> None:
    """Test getting human-readable description of set bits."""
    # Test with no bits set
    desc = BitmaskHelper.get_description(0, bitmask_dict)
    assert desc == "None"

    # Test with single bit set
    desc = BitmaskHelper.get_description(1, bitmask_dict)
    assert desc == "Option 1"

    # Test with multiple bits set
    desc = BitmaskHelper.get_description(7, bitmask_dict)
    assert desc == "Option 1, Option 2, Option 3"

    # Test with all bits set
    desc = BitmaskHelper.get_description(31, bitmask_dict)
    assert desc == "Option 1, Option 2, Option 3, Option 4, Option 5"

    # Test with some bits set
    desc = BitmaskHelper.get_description(18, bitmask_dict)
    assert desc == "Option 2, Option 5"

    # Test with bits not in the dictionary
    desc = BitmaskHelper.get_description(64, bitmask_dict)  # 2^6 = 64
    assert desc == "None"
