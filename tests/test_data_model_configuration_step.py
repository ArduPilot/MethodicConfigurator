...
from ardupilot_methodic_configurator.data_model_configuration_step import (
    ConfigurationStepProcessor,
    get_auto_importable_parameter_names,
)
...
class TestConfigurationStepProcessorWorkflows:
    ...
    def test_auto_importable_names_include_only_matching_non_default_known_parameters(self) -> None:
        """
        Auto-import eligibility identifies only parameters that the step can safely import.

        GIVEN: A step with BATT and GPS auto-import patterns
        AND: FC values containing a changed BATT parameter, a default GPS parameter,
        an unmatched parameter, and an unknown parameter
        WHEN: Auto-importable parameter names are calculated
        THEN: Only the changed known BATT parameter is returned
        """
        step = {"autoimport_nondefault_regexp": ["BATT.*", "GPS.*"]}
        fc_parameters = {
            "BATT_MONITOR": 4.0,
            "GPS_TYPE": 0.0,
            "SERIAL1_BAUD": 115200.0,
            "BATT_UNKNOWN": 3.0,
        }
        defaults = {
            "BATT_MONITOR": Par(value=0.0),
            "GPS_TYPE": Par(value=0.0),
            "SERIAL1_BAUD": Par(value=57600.0),
        }

        imported_names = get_auto_importable_parameter_names(step, fc_parameters, defaults)

        assert imported_names == {"BATT_MONITOR"}
    ...
    def test_autoimport_results_do_not_leak_between_configuration_steps(self, processor) -> None:
        """
        Auto-imported parameter names belong only to the step that produced them.

        GIVEN: One configuration step auto-imports a non-default FC parameter
        AND: A later configuration step has no matching auto-import rule
        WHEN: Both steps are processed sequentially
        THEN: The first result reports its imported parameter
        AND: The later result reports no imported parameters
        """
        first_file = "10_first_step.param"
        second_file = "11_second_step.param"
        processor.local_filesystem.configuration_steps = {
            first_file: {"autoimport_nondefault_regexp": ["BATT.*"]},
            second_file: {},
        }
        processor.local_filesystem.file_parameters = {
            first_file: {},
        ...
        processor.local_filesystem.param_default_dict = {"BATT_MONITOR": Par(value=0.0, comment="Default monitor")}
        fc_parameters = {"BATT_MONITOR": 4.0}

        first_result = processor.process_configuration_step(first_file, fc_parameters)
        second_result = processor.process_configuration_step(second_file, fc_parameters)

        assert first_result[6] == {"BATT_MONITOR"}
        assert second_result[6] == set()
