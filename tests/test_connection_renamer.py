...
def test_generate_renames_serial(connection_params) -> None:
    ...
    # Check that only SERIAL1 parameters are renamed, not SERIAL2
    for old_name, new_name in expected_renames.items():
        assert renames[old_name] == new_name

    assert "SERIAL2_PROTOCOL" not in renames
    assert "SERIAL2_BAUD" not in renames
...
def test_apply_renames_without_duplicates(connection_params) -> None:
    ...
    updated_params, new_names, renamed_pairs = ConnectionRenamer.apply_renames(params, "CAN2")

    # Check that the parameters were renamed correctly
    assert "CAN_P2_DRIVER" in updated_params
    assert "CAN_D2_PROTOCOL" in updated_params
    assert "CAN_P1_DRIVER" not in updated_params
    assert "CAN_D1_PROTOCOL" not in updated_params

    # Check that only CAN parameters were renamed
    assert "SERIAL1_PROTOCOL" in updated_params
    assert "SERIAL2_BAUD" in updated_params

    # Check new names set
    assert "CAN_P2_DRIVER" in new_names
    assert "CAN_D2_PROTOCOL" in new_names
...
def test_apply_renames_with_duplicates(connection_params) -> None:
    ...
    # Check that the existing CAN_P2_DRIVER wasn't affected
    assert "CAN_P2_DRIVER" in updated_params
    assert updated_params["CAN_P2_DRIVER"].value == 2.0
...
def test_apply_renames_with_variables(connection_params) -> None:
    ...
    updated_params, new_names, renamed_pairs = ConnectionRenamer.apply_renames(params, "selected_can", variables)

    # Check that the parameters were renamed correctly using the evaluated variable
    assert "CAN_P3_DRIVER" in updated_params
    assert "CAN_D3_PROTOCOL" in updated_params
    assert "CAN_P1_DRIVER" not in updated_params
    assert "CAN_D1_PROTOCOL" not in updated_params
