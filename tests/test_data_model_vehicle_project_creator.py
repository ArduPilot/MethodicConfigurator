...
class TestBinLogImportHelpers:
    ...
    def test_template_dir_for_bin_import_rejects_path_traversal(self, tmp_path) -> None:
        """A firmware name cannot select a template outside the templates directory."""
        templates = tmp_path / "templates"
        outside_template = tmp_path / "outside" / "empty_4.6.x"
        outside_template.mkdir(parents=True)

        with (
            patch.object(LocalFilesystem, "get_templates_base_dir", return_value=str(templates)),
            pytest.raises(VehicleProjectCreationError),
        ):
            VehicleProjectCreator.template_dir_for_bin_import("../outside", 4, 6)
