...
class TestMotorTestFunctionality:
    ...
    def test_user_can_test_motors_in_sequence(self, flight_controller) -> None:
        ...
        success, error_msg = flight_controller.test_motors_in_sequence(throttle_percent, timeout_seconds)
    ...
    def test_motor_count_detection_works_for_different_frames(self, flight_controller) -> None:
        """
        Motor count detection works correctly for different frame types.

        GIVEN: Different frame configurations are set
        WHEN: Motor count is requested for each frame type
        THEN: The correct number of motors should be returned for each frame
        """
        # Arrange & Act & Assert: Test different frame configurations
        test_cases = [
            (1, 1, 4),  # QUAD X = 4 motors
            (1, 0, 4),  # QUAD PLUS = 4 motors
            (2, 1, 6),  # HEXA X = 6 motors
            (3, 1, 8),  # OCTO X = 8 motors
            (7, 0, 3),  # TRICOPTER = 3 motors
        ]

        for frame_class, frame_type, expected_motors in test_cases:
            flight_controller.fc_parameters = {
                "FRAME_CLASS": frame_class,
                "FRAME_TYPE": frame_type,
            }

            motor_count = flight_controller.get_motor_count_from_frame()
            assert motor_count == expected_motors
...
class TestMotorTestCommandSending:
    ...
    def test_motor_count_detection_works_for_different_frames(self, flight_controller) -> None:
        ...
        GIVEN: Different FRAME_CLASS and FRAME_TYPE combinations are configured
        WHEN: Motor count is calculated from frame parameters
        THEN: The correct number of motors should be returned for each frame type
        AND: Standard frame types should be properly recognized
        """
        # Arrange & Act & Assert: Test frame motor counting
        valid_frames = [
            (1, 0, 4),  # QUAD PLUS
            (1, 1, 4),  # QUAD X
            (2, 0, 6),  # HEXA PLUS
        ...
        for frame_class, frame_type, expected_motors in valid_frames:
            flight_controller.fc_parameters = {
                "FRAME_CLASS": frame_class,
                "FRAME_TYPE": frame_type,
            }
            motor_count = flight_controller.get_motor_count_from_frame()
            assert motor_count == expected_motors, f"Frame {frame_class}/{frame_type} should have {expected_motors} motors"
