...
class TestApjLoading:
    ...
    def test_user_can_load_an_external_only_apj_file(self, tmp_path: Path) -> None:
        """
        An APJ may contain only an external-flash image.

        GIVEN: An APJ descriptor with an empty internal image and external payload
        WHEN: It is loaded for firmware upload
        THEN: The empty internal region is accepted and the external payload is preserved
        """
        ...
        assert image.metadata.image_size == 0
        assert image.metadata.extf_image_size == len(external_image)
...
class TestStateMachine:
    ...
    def test_dual_region_upload_can_return_to_erasing_for_the_internal_image(self) -> None:
        """The state machine permits external verification before internal flashing."""
        stage = fw.UploadStage.IDLE
        for target in (
            fw.UploadStage.INSPECTING,
            fw.UploadStage.ENTERING_BOOTLOADER,
