    ...
    def test_user_can_write_setting_values(self) -> None:
        ...
        with (
            ...
            patch(
                ...
                {"gui_complexity": "normal", "auto_open_doc_in_browser": True},
            ),
        ):
            mock_get_config.return_value = (
                {"gui_complexity": "normal", "auto_open_doc_in_browser": True},
            ...
            mock_set_settings.assert_called_with({"gui_complexity": "simple", "auto_open_doc_in_browser": True})
...
class TestMotorTestSettings:
    """Test motor test configuration settings management."""

    def test_user_can_get_default_motor_test_duration(self) -> None:
        """
        User can retrieve the default motor test duration setting.

        GIVEN: A fresh program settings configuration
        WHEN: User requests the motor test duration
        THEN: The default duration should be returned as a float
        AND: Duration should be within safety limits
        """
        # Arrange: Mock the get_setting method to return default
        with patch.object(ProgramSettings, "get_setting", return_value=5.0):
            # Act: Get motor test duration
            duration = ProgramSettings.get_motor_test_duration()

            # Assert: Default duration is returned and is valid
            assert duration == 5.0
            assert isinstance(duration, float)
            assert 0.1 <= duration <= 10.0

    def test_user_can_set_valid_motor_test_duration(self) -> None:
        """
        User can set a valid motor test duration within safety limits.

        GIVEN: A user wants to configure motor test duration
        WHEN: User sets a duration within valid range (0.1-10.0 seconds)
        THEN: The duration should be saved successfully
        """
        # Arrange: Mock the set_setting method
        with patch.object(ProgramSettings, "set_setting") as mock_set_setting:
            # Act: Set valid duration
            ProgramSettings.set_motor_test_duration(3.5)

            # Assert: Setting was called with correct parameters
            mock_set_setting.assert_called_once_with("motor_test_duration", 3.5)

    def test_user_cannot_set_unsafe_motor_test_duration(self) -> None:
        """
        User cannot set motor test duration outside safety limits.

        GIVEN: A user attempts to set an unsafe motor test duration
        WHEN: User sets duration outside valid range (0.1-10.0 seconds)
        THEN: The setting should be rejected and not saved
        """
        # Arrange: Mock the set_setting method
        with patch.object(ProgramSettings, "set_setting") as mock_set_setting:
            # Act: Try to set unsafe durations
            ProgramSettings.set_motor_test_duration(0.05)  # Too short
            ProgramSettings.set_motor_test_duration(15.0)  # Too long

            # Assert: Setting was never called for unsafe values
            mock_set_setting.assert_not_called()

    def test_user_can_get_default_motor_test_throttle_percentage(self) -> None:
        """
        User can retrieve the default motor test throttle percentage.

        GIVEN: A fresh program settings configuration
        WHEN: User requests the motor test throttle percentage
        THEN: The default percentage should be returned as an integer
        AND: Percentage should be within valid range
        """
        # Arrange: Mock the get_setting method to return default
        with patch.object(ProgramSettings, "get_setting", return_value=10):
            # Act: Get motor test throttle percentage
            throttle = ProgramSettings.get_motor_test_throttle_pct()

            # Assert: Default throttle is returned and is valid
            assert throttle == 10
            assert isinstance(throttle, int)
            assert 1 <= throttle <= 100

    def test_user_can_set_valid_motor_test_throttle_percentage(self) -> None:
        """
        User can set a valid motor test throttle percentage.

        GIVEN: A user wants to configure motor test throttle percentage
        WHEN: User sets a percentage within valid range (1-100%)
        THEN: The percentage should be saved successfully
        """
        # Arrange: Mock the set_setting method
        with patch.object(ProgramSettings, "set_setting") as mock_set_setting:
            # Act: Set valid throttle percentage
            ProgramSettings.set_motor_test_throttle_pct(25)

            # Assert: Setting was called with correct parameters
            mock_set_setting.assert_called_once_with("motor_test_throttle_pct", 25)

    def test_user_cannot_set_invalid_motor_test_throttle_percentage(self) -> None:
        """
        User cannot set motor test throttle percentage outside valid range.

        GIVEN: A user attempts to set an invalid throttle percentage
        WHEN: User sets percentage outside valid range (1-100%)
        THEN: The setting should be rejected and not saved
        """
        # Arrange: Mock the set_setting method
        with patch.object(ProgramSettings, "set_setting") as mock_set_setting:
            # Act: Try to set invalid throttle percentages
            ProgramSettings.set_motor_test_throttle_pct(0)  # Too low
            ProgramSettings.set_motor_test_throttle_pct(150)  # Too high

            # Assert: Setting was never called for invalid values
            mock_set_setting.assert_not_called()
...
class TestMotorDiagramManagement:
    ...
    def test_user_can_get_motor_diagram_filepath_for_existing_frame(self) -> None:
        ...
        with (
            ...
            filepath = ProgramSettings.motor_diagram_filepath(frame_class=1, frame_type=1)
    ...
    def test_user_gets_default_diagram_when_specific_frame_not_found(self) -> None:
        ...
        with (
            ...
            filepath = ProgramSettings.motor_diagram_filepath(frame_class=99, frame_type=99)

            # Assert: Default diagram filename is in the path
            assert filepath.endswith("m_01_01_quad_x.svg")
            assert "images" in filepath
    ...
    def test_motor_diagram_filepath_works_in_compiled_executable(self) -> None:
        ...
        with (
            ...
            def getattr_side_effect(_obj, attr, default=None) -> bool:
                if attr == "frozen":
                    return True
                return default
...
class TestGUIComplexitySettings:
    ...
    def test_user_can_manage_gui_complexity_setting(self) -> None:
        ...
        with (
            ...
            mock_get_config.return_value = ({"gui_complexity": "normal"}, "pattern", "replacement")
            ProgramSettings.set_setting("gui_complexity", "simple")
            mock_set_settings.assert_called_with({"gui_complexity": "simple"})
