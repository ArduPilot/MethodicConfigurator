...
- **`--template-dir PATH`**: Optional template directory to use with `--bin-log`. If omitted,
  ...
  Bundled empty templates currently include:
...
- **`-t` or `--vehicle-type`**: The type of the vehicle.
  ...
  Without `--bin-log`, the default is 'ArduCopter'. With `--bin-log`, the type is derived from the log unless `-t` is given.
  With `--bin-log`, this value validates both the vehicle type reported by the log and the selected template, and records
  the explicitly selected type in the new project (for example, `Heli` uses the compatible `ArduCopter` template).
