...
- **`Change Reason` expressions**: When the field contains a Python `if...else` conditional expression it is evaluated at runtime and the appropriate branch is used as the displayed reason. If evaluation fails the raw string is used as a fallback and a warning is logged.
