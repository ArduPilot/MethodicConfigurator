...
an icon-only `⟳` refresh button, a Select all button, and a sortable `ttk.Treeview` with
**Name**, **Type**, and **Size** columns.
The remote entry is initialized to `/APM/LOGS/`; Enter in that field performs
the same operation as the refresh button. Double-clicking a directory navigates
into it. The local panel additionally provides Browse.
...
dialog fallback when inline editing is unavailable. F5 refreshes the panel that
has keyboard focus. The transfer buttons are enabled only when their
corresponding panel contains a selected file or directory. The bottom action
bar contains four equally spaced, equal-width buttons in this order:
**Download last .bin log file**, **Download selected**, **Upload selected**, and
**Close**. During an active transfer, **Close** becomes **Cancel operation**.
...
- **Close**, which closes the browser while idle or cancels an active transfer.

Delete and rename are keyboard actions on the selected panel rather than
bottom-row buttons.
...
When launched as a standalone module, the window creates its own Tk root,
connects the `FlightController` using the requested device or normal
autodetection, and defers the initial listings with `after_idle` so the window
can render before the potentially blocking remote directory request.
...
callback into aggregate byte-oriented batch progress. The Cancel operation
button signals the active transfer to stop at the next MAVFTP payload/progress
callback and failed transfers terminate their MAVFTP session. MAVFTP invokes
the download progress callback even when the flight controller does not report
a remote file size, so those downloads remain cancellable. Zero-byte files
count as one progress unit. Empty directories are created without file
progress units.
...
filename and numeric-size sorting, selection-dependent transfer buttons,
Select all/Ctrl+A, parent-directory button and Backspace navigation, F5 panel
refresh, remote refresh/Enter behavior, legacy destination selectors, and
modal integration. `tests/test_backend_mavftp.py` covers progress callbacks
for downloads whose remote size is unknown, preserving cancellation support.
