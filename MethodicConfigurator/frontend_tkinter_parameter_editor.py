...
class DocumentationFrame:  # pylint: disable=too-few-public-methods
    ...
    def update_documentation_labels(self, current_file: str):
        self.current_file = current_file
        if current_file:
            frame_title = _(f"{current_file} Documentation")
        else:
...
class ParameterEditorWindow(BaseWindow):  # pylint: disable=too-many-instance-attributes
    ...
    def __create_conf_widgets(self, version: str):
        ...
        self.file_selection_combobox = AutoResizeCombobox(file_selection_frame,
                                                          ...
                                                          _("files in the selected vehicle directory\nIt will automatically ")
                                                          _("advance to the next file once the current file is uploaded to the ")
                                                          _("fight controller"),
    ...
    def __create_parameter_area_widgets(self):
        ...
        show_tooltip(annotate_params_checkbox, _("Annotate ArduPilot parameter documentation metadata into the intermediate ")
                     _("parameter files\n")
                     _("The files will be bigger, but all the existing parameter documentation will be included inside"))

        # Create upload button
        upload_selected_button = ttk.Button(buttons_frame, text=_("Upload selected params to FC, and advance to next param file"),
    ...
    def __do_tempcal_imu(self, selected_file:str):
        ...
        if selected_file == tempcal_imu_result_param_filename:
            if messagebox.askyesno(_("IMU temperature calibration"),
                                    _(f"If you proceed the {tempcal_imu_result_param_filename}\n")
                                    _("will be overwritten with the new calibration results.\n")
                                    _("Do you want to provide a .bin log file and\n")
                                    _("run the IMU temperature calibration using it?")):
                ...
                if filename:
                    messagebox.showwarning(_("IMU temperature calibration"), _("Please wait, this can take a really long time and\n")
                                           _("the GUI will be unresponsive until it finishes."))
    ...
    def __should_copy_fc_values_to_file(self, selected_file: str):
        ...
        if auto_changed_by and self.flight_controller.fc_parameters:
            if messagebox.askyesno(_("Update file with values from FC?"),
                                   ...
                                   _(f"Should the FC values now be copied to the {selected_file} file?")):
    ...
    def __should_download_file_from_url(self, selected_file: str):
        ...
        if url and local_filename:
            ...
            if messagebox.askyesno(_("Download file from URL"),
                                _(f"Should the {local_filename} file be downloaded from the URL\n{url}?")):
                if not self.local_filesystem.download_file_from_url(url, local_filename):
                    messagebox.showerror(_("Download failed"),
                                         _(f"Failed to download {local_filename} from {url}, please download it manually"))
    ...
    def __should_upload_file_to_fc(self, selected_file: str):
        ...
        if local_filename and remote_filename:
            if not self.local_filesystem.vehicle_configuration_file_exists(local_filename):
                messagebox.showerror(_("Will not upload any file"), _(f"Local file {local_filename} does not exist"))
                return
            if self.flight_controller.master:
                if messagebox.askyesno(_("Upload file to FC"),
                        _(f"Should the {local_filename} file be uploaded to the flight controller as {remote_filename}?")):
                    ...
                    if not self.flight_controller.upload_file(local_filename, remote_filename,
                        ...
                        messagebox.showerror(_("Upload failed"),
                                             _(f"Failed to upload {local_filename} to {remote_filename},") \
                                             _(" please upload it manually"))
    ...
    def download_flight_controller_parameters(self, redownload: bool = False):
        self.param_download_progress_window = ProgressWindow(self.main_frame, (_("Re-d") if redownload else _("D")) + \
                                                             _("ownloading FC parameters"), _("Downloaded {} of {} parameters"))
    ...
    def upload_params_that_require_reset(self, selected_params: dict):
        ...
        for param_name, param in selected_params.items():
            try:
                ...
                logging_error(_("Failed to set parameter %s: %s"), param_name, e)
                messagebox.showerror(_("ArduPilot methodic configurator"), _(f"Failed to set parameter {param_name}: {e}"))
    ...
    def __reset_and_reconnect(self, fc_reset_required, fc_reset_unsure):
        if not fc_reset_required:
            if fc_reset_unsure:
                ...
                fc_reset_required = messagebox.askyesno(_("Possible reset required"), _(f"{(', ').join(fc_reset_unsure)} parameter")
                                                        _("(s) potentially require a reset\nDo you want to reset the ArduPilot?"))
    ...
    def write_changes_to_intermediate_parameter_file(self):
        if self.parameter_editor_table.get_at_least_one_param_edited():
            if messagebox.askyesno(_("One or more parameters have been edited"),
                                   _(f"Do you want to write the changes to the {self.current_file} file?")):
    ...
    def write_summary_files(self):
        ...
        summary_message = _(f"Methodic configuration of {len(annotated_fc_parameters)} parameters complete:\n\n" )\
            ...
            _(f"{len(non_default__read_only_params)} non-default read-only parameters - ") \
            _("ignore these, you can not change them\n\n") \
            _(f"{len(non_default__writable_calibrations)} non-default writable sensor-calibrations - ") \
            _("non-reusable between vehicles\n\n") \
            _(f"{len(non_default__writable_non_calibrations)} non-default writable non-sensor-calibrations - ") \
            _("these can be reused between similar vehicles")
        messagebox.showinfo(_("Last parameter file processed"), summary_message)
    ...
    def write_summary_file(self, param_dict: dict, filename: str, annotate_doc: bool):
        ...
        if param_dict:
            if self.local_filesystem.vehicle_configuration_file_exists(filename):
                should_write_file = messagebox.askyesno(_("Overwrite existing file"),
                                                        _(f"{filename} file already exists.\nDo you want to overwrite it?"))
    ...
    def write_zip_file(self, files_to_zip: List[Tuple[bool, str]]):
        ...
        if self.local_filesystem.zip_file_exists():
            should_write_file = messagebox.askyesno(_("Overwrite existing file"),
                                                    _(f"{zip_file_path} file already exists.\nDo you want to overwrite it?"))
        if should_write_file:
            ...
            messagebox.showinfo(_("Parameter files zipped"), _("All relevant files have been zipped into the \n")
                                ...
                                _("Configuration Blog post on discuss.ardupilot.org."))
        return should_write_file
